...
object SavingsPrefs {
    ...
    /**
     * 목표 금액을 다 채워서 "부자 되세요~" 축하 연출을 보여준 뒤, 다음 저축 사이클을 위해
     * 모은 금액만 비운다. lastProcessedMonth는 그대로 남겨둬서, 이후 실제 월별 정산 로직이
     * 지나간 달을 다시 이중으로 적립하지 않도록 한다.
     */
    fun resetSavedOnly(context: Context) {
        prefs(context).edit()
            .remove(KEY_TOTAL_SAVED)
            .remove(KEY_LAST_CREDITED)
            .apply()
